"""watchtower end to end: intake, bus, worker, approval, commit.
...
from watchtower import agents
from watchtower.app import runtime
...
def payload(**extra) -> dict:
    base = {
        ...
        "packages": [{"name": "openssl", "version": "3.0.2", "release": "jammy"}],
        "advisories": [{"cve": "CVE-1", "package": "openssl", "fixed_upstream": "3.0.8"}],
...
def test_the_early_exit_costs_no_generation():
    ...
    rt.submit(
        "r1",
        "e1",
        payload(**{"host": "someone-elses-box.example.com", "scope": ["workstation.local"]}),
    )
    rt.drain()
    row = rt.run_row("r1")
    assert row["status"] == api.DONE
    assert row["result"]["refused"] is True
    assert model.calls == []
